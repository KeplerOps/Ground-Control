...
public interface AssetLinkRepository extends JpaRepository<AssetLink, UUID> {
    ...
    @Query("SELECT l FROM AssetLink l JOIN FETCH l.asset" + " WHERE l.id = :id AND l.asset.project.id = :projectId")
    Optional<AssetLink> findByIdWithAssetAndProjectId(@Param("id") UUID id, @Param("projectId") UUID projectId);
    ...
    boolean existsByAssetIdAndTargetTypeAndTargetEntityIdAndLinkType(
            UUID assetId, AssetLinkTargetType targetType, UUID targetEntityId, AssetLinkType linkType);
    ...
    List<AssetLink> findByTargetTypeAndTargetEntityIdAndProjectId(
            @Param("targetType") AssetLinkTargetType targetType,
