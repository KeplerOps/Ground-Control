...
class OpenAiEmbeddingProviderTest {
    ...
    void isAvailable_returnsTrueWhenApiKeySet() {
        var props = new EmbeddingProperties("openai", "sk-test-key", "https://api.openai.com/v1", "model", 1536, 100);
    ...
    void isAvailable_returnsFalseWhenApiKeyBlank() {
        var props = new EmbeddingProperties("openai", "", "https://api.openai.com/v1", "model", 1536, 100);
        var provider = new OpenAiEmbeddingProvider(props);
    ...
    void isAvailable_returnsFalseWhenApiKeyNull() {
        var props = new EmbeddingProperties("openai", null, "https://api.openai.com/v1", "model", 1536, 100);
        var provider = new OpenAiEmbeddingProvider(props);
        assertThat(provider.isAvailable()).isFalse();
    }
    ...
    void getModelId_returnsConfiguredModel() {
        var props = new EmbeddingProperties(
                "openai", "sk-test", "https://api.openai.com/v1", "text-embedding-3-small", 1536, 100);
        var provider = new OpenAiEmbeddingProvider(props);
    ...
    void getDimensions_returnsConfiguredDimensions() {
        var props = new EmbeddingProperties("openai", "sk-test", "https://api.openai.com/v1", "model", 768, 100);
        var provider = new OpenAiEmbeddingProvider(props);
