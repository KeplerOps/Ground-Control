...
server.tool(
  ...
  "Register a new pack version in the registry catalog. This adds the pack as a discoverable, installable artifact.",
  {
    ...
    source_url: z.string().max(2000).optional().describe("Origin URL (registry, git repo, etc.)"),
    checksum: z.string().max(128).optional().describe("SHA-256 content hash for integrity verification"),
    signature_info: z.record(z.string(), z.unknown()).optional().describe("Signature metadata"),
    compatibility: z.record(z.string(), z.unknown()).optional().describe("Compatibility constraints (e.g. minVersion, maxVersion)"),
    dependencies: z.array(z.record(z.string(), z.unknown())).optional().describe("Pack dependencies [{packId, versionConstraint}]"),
...
server.tool(
  ...
  "Update metadata of a pack registry entry.",
  {
    ...
    checksum: z.string().max(128).optional().describe("Updated checksum"),
    signature_info: z.record(z.string(), z.unknown()).optional().describe("Updated signature metadata"),
    compatibility: z.record(z.string(), z.unknown()).optional().describe("Updated compatibility constraints"),
    dependencies: z.array(z.record(z.string(), z.unknown())).optional().describe("Updated dependencies"),
...
server.tool(
  ...
  "Create a trust policy with declarative rules for evaluating pack trust. Rules match against pack fields (publisher, packId, packType, sourceUrl, checksum) using operators (EQUALS, NOT_EQUALS, CONTAINS, MATCHES_PATTERN, IN_LIST).",
  {
    ...
    rules: z.array(z.object({
      field: z.string().describe("Pack field to match: publisher, packId, packType, sourceUrl, checksum"),
...
server.tool(
  ...
  "Update a trust policy.",
  {
    ...
    rules: z.array(z.object({
      field: z.string().describe("Pack field to match"),
      operator: z.enum(TRUST_POLICY_RULE_OPERATORS).describe("Comparison operator"),
      value: z.string().describe("Value to compare against"),
...
server.tool(
  ...
  "Install a pack through the registry with trust evaluation. Resolves the version, checks compatibility, evaluates trust policy, and delegates to the type-specific installer. Produces an auditable install record.",
  {
    ...
    })).describe("Control pack entry definitions"),
    project: z.string().optional().describe("Project identifier"),
  },
  async ({ project, ...data }) => {
    try {
...
server.tool(
  ...
  "Upgrade a pack through the registry with trust evaluation. Same flow as install but delegates to the upgrade path.",
  {
    pack_id: z.string().max(200).describe("Pack identity to upgrade"),
    ...
    })).describe("Control pack entry definitions for new version"),
    project: z.string().optional().describe("Project identifier"),
  },
  async ({ project, ...data }) => {
    try {
