...
- Scope: `domain/requirements/state/` and `domain/verification/state/` only (pure enums with no framework annotations). Other `state/` packages (`assets`, `controls`, `riskscenarios`, `plugins`, `controlpacks`, `packregistry`, `threatmodels`) contain L0 value enums not covered by ESC.
