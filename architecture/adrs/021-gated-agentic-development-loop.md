...
>
> **Amended by issue #801 (2026-05-05):** Phase A's TDD step gains a narrow documentation-only carve-out (no executable behavior in the diff + every clause/criterion protected by a named structural gate; declared in the plan and re-stated on the issue thread; substring/snapshot tests are not gates). Phase D's traceability reconciliation gains a backfill rule for runs whose diff finalizes a requirement whose structural implementation lives in pre-existing files shipped under a sibling requirement — IMPLEMENTS links are backfilled onto those artifacts of record, bounded by the requirement's concrete subject matter. See `skills/implement/SKILL.md` Step 4.4 and Step 16 for the operative prose, and `architecture/notes/implement-docs-only-preexisting-traceability-guardrails.md` for the preflight design context.
