...
4. **If the documentation-only carve-out from Step 4.4 was declared**, re-validate it against the *actual* diff right now. Two checks, both required:
   1. **Path check (necessary, not sufficient).** Run `git diff --name-only <base-ref>...HEAD` and confirm every changed path is documentation. The documentation set is intentionally narrow: `*.md`, `architecture/adrs/**`, `architecture/notes/**`, `docs/**`, `CHANGELOG.md`, `README.md`, `skills/**/*.md`, and equivalent doc-only locations declared by the repo. Any path outside that set — `*.java`, `*.py`, `*.ts`, `*.tsx`, `*.js`, `*.kts`, `*.gradle`, `*.yaml`/`*.yml` (workflows, configs), `*.sh`, `*.sql`, `Dockerfile`, `Makefile`, `*.json` (policies, package manifests, lockfiles), etc. — invalidates the carve-out outright.
   2. **Content check (the path check is not enough — a doc file can carry executable behavior).** Run `git diff <base-ref>...HEAD -- <each-path-from-step-1>` and read the actual content of every diff hunk. If any hunk introduces executable behavior — code fences whose contents are intended to be executed by tooling, embedded YAML that a code path parses and acts on, schema/grammar/policy data consumed by a runtime parser, runnable test fixtures, or any other line of static text whose meaning is "what the program should do at runtime" — the carve-out is invalidated for those clauses, and the mandatory red-green loop applies (write the failing test against the parser/consumer, then make the doc edit pass it).
   3. The carve-out passes Step 6 only when BOTH checks pass: every path is in the documentation set AND no diff hunk introduces executable behavior. If either check fails, revert to the mandatory red-green loop for the failing portion before declaring the gate passed.
