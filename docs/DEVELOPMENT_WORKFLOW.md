...
- **Steps 13–14** are the post-push review phase: `/review-tests` catches false-assurance tests, then one final CI pass closes out. Step 13 mirrors Step 6.5's record-keeping contract — every cycle ends with a `gc_post_decision_record(reviewer: "test-quality", findings: [...])` call, and a clean cycle (`findings: []`) is the structured advance-to-Step-14 signal only after the post returns `ok: true`; on `ok: false` the workflow fixes the underlying tooling issue and retries the post rather than advancing without the durable marker (issue #884). **The whole point of the review is to fix the tests** — when the skill returns findings, the parent fixes every finding in the same agent turn (classify, fix, self-verify, commit/push, post decision record with dispositions, re-invoke) without stopping or echoing the findings to the user; surfacing findings back to the user as a status report is the failure mode the user observed after #884 v1 shipped and is forbidden by Step 13. Agent-honored 3-cycle cap per issue, aligned with ADR-029's reviewer cap — unlike the codex cap this one is workflow discipline rather than server-side refusal (`gc_post_decision_record` does not refuse cycle 4 for `reviewer: "test-quality"`).
