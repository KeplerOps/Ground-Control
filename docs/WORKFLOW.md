...
4. **Test quality review** (Step 13, agent-honored 3-cycle cap) — `/review-tests` catches assertion-free tests, mock-only assertions, integration-as-unit tests, and tests that can't detect regressions. The cap is workflow discipline, not server-side: unlike `gc_codex_review`, `gc_post_decision_record` does not refuse cycle 4 for `reviewer: "test-quality"`. The whole point of the review is to **fix** the tests, not to file a status report on them — when the skill returns findings, the parent `/implement` workflow fixes every finding in the same agent turn (Case A: classify, fix, self-verify, commit/push, post decision record, re-invoke) without stopping or echoing the findings back to the user. Every cycle ends with a `gc_post_decision_record(reviewer: "test-quality", findings: [...])` post; a clean cycle posts `findings: []` and, once the call returns `ok: true`, is the structured advance-to-Step-14 signal — there is no acknowledgment turn between Step 13 and Step 14 (issue #884). On `ok: false`, the workflow fixes the underlying tooling issue and retries the post rather than advancing without the durable marker.
